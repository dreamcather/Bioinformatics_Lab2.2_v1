// Lab2.2_v1.cpp: определяет точку входа для консольного приложения.
//
...
int HamingDistnance(int step, int shift)
{
	int res = 0;
	for (int i = 0; i < Size_Motiv; i++)
	{
...
int MinPosition(int step)
{
	int min = Size_Motiv;
	for (int i = 0; i < Exaples_Lenght - Size_Motiv + 1;i++)
	{
		int res = HamingDistnance(step, i);
		if (res < min)
			min = res;
	}
	return min;
